...
function generateSafeUserId() {
    $db = getDB();
    $maxAttempts = 10;
    
    for ($i = 0; $i < $maxAttempts; $i++) {
        // สร้าง ID ที่เล็กกว่า INT max (2147483647)
        // ใช้ 6-9 หลัก เพื่อความปลอดภัย
        ...
        if (!$stmt->fetch()) {
            return $userId; // ไม่ซ้ำ
        }
    }
    
    throw new Exception("ไม่สามารถสร้าง User ID ได้");
}

function generateSafePollId() {
    $db = getDB();
    $maxAttempts = 10;
    
    for ($i = 0; $i < $maxAttempts; $i++) {
        // สร้าง ID ที่เล็กกว่า INT max (2147483647)
        $pollId = rand(100000, 999999999);
        ...
        if (!$stmt->fetch()) {
            return $pollId; // ไม่ซ้ำ
        }
    }
    
    throw new Exception("ไม่สามารถสร้าง Poll ID ได้");
}

function generateSafeResponseId() {
    $db = getDB();
    $maxAttempts = 10;
    
    for ($i = 0; $i < $maxAttempts; $i++) {
        // สร้าง ID ที่เล็กกว่า INT max (2147483647)
        $responseId = rand(100000, 999999999);
        
        // เช็คว่าซ้ำหรือไม่
        $stmt = $db->prepare("SELECT id FROM responses WHERE id = ? LIMIT 1");
        $stmt->execute([$responseId]);
        
        if (!$stmt->fetch()) {
            return $responseId; // ไม่ซ้ำ
        }
    }
    
    throw new Exception("ไม่สามารถสร้าง Response ID ได้");
}

function generateSafeVoteId() {
    $db = getDB();
    $maxAttempts = 10;
    
    for ($i = 0; $i < $maxAttempts; $i++) {
        // สร้าง ID ที่เล็กกว่า INT max
        // ใช้ string format เพราะ votes.id เป็น VARCHAR
        $voteId = time() . rand(100, 999);
        
        // เช็คว่าซ้ำหรือไม่
        $stmt = $db->prepare("SELECT id FROM votes WHERE id = ? LIMIT 1");
        $stmt->execute([$voteId]);
        
        if (!$stmt->fetch()) {
            return $voteId; // ไม่ซ้ำ
        }
        
        usleep(1000); // รอเล็กน้อยเพื่อให้ time() เปลี่ยน
    }
    
    throw new Exception("ไม่สามารถสร้าง Vote ID ได้");
}
